<template>
            ...
            <v-card-actions>
                ...
                <v-menu class="border border-surface-light">
                    ...
                    <v-list class="border border-surface-light" density="compact">
                        <v-list-item @click="toggleDraft()" prepend-icon="mdi-file-edit" :class="document.data.draft ? 'text-orange': ''">
                                {{document.data.draft ? 'Release doc': 'Stage doc'  }}
                        </v-list-item>
                        <v-list-item @click="archiveDocument" prepend-icon="mdi-archive">
                            Archive doc
                        </v-list-item>
                        <v-list-item class="text-error" @click="deleteDocument" prepend-icon="mdi-trash-can">
    ...
    <div class="document-create position-relative top-0 left-0 right-0 h-100 ml-6">
        <Transition v-bind="$fadeTransition">
            <div v-if="isLoadingTimeout && isLoading" class="skeleton-loader-container position-absolute top-0 left-0 right-0 w-100 h-100 d-flex justify-center align-center">
                <v-progress-circular
                    class="mx-auto"
                    color="primary"
                    indeterminate
...
export default {
    ...
    data: () => ({
      ...
      documentTypes: [
        'product', 'feature', 'roadmap', 'goal', 'idea', 'risk',
        'persona', 'need', 'journey', 'job to be done', 'insight', 'interview'
      ],
      rules: {
    ...
    methods: {
        ...
        async fetchDocument(id, version = null) {
            ...
            this.isLoading = false;
            if (version) { this.isEditable = false } else { this.isEditable = true }
        },

        async createDocument() {
            console.log('create-doc')
            const createdDoc = await this.$store.dispatch('createDocument', { data: this.document.data});
            this.document.id = createdDoc.id;
            this.$router.replace({ path: `/document/${this.document.id}`})
       },

        async saveDocument() {
            if (this.isLoading == true || this.isEditorModified === false) {return}
        ...
        async populateTemplate(type) {
            ...
            this.documentTemplate = await this.$store.state.templates.find(t => t.name === type);
    ...
    watch: {
        ...
        'document.data': {
            async handler(){
                ...
                if (!this.isLoading && this.document.data.content !== this.previousContent) {
                ...
                if (this.isLoading) { // this can probably get cleaned up...
                    this.isLoading = false;
                    return
                }
            },
        ...
        '$route': {
            async handler(to, from) {
                if( this.isEditorModified) {await this.saveDocument()}
        ...
        isLoading(newValue) { // this is only here to hide ugly blinking effect on milkdown editor init
            if (newValue === false && this.isLoadingTimeout === false) {
                
                this.isLoadingTimeout = false; // Reset timeout when loading is false
            } else if (newValue === true) {
                setTimeout(() => {
                    if (this.isLoading) { // Check if still loading after 1 second
...
.input-container {
    background: linear-gradient(to bottom, rgba(var(--v-theme-background), 1) 0%, rgba(var(--v-theme-background), 1) 40%, rgba(var(--v-theme-background), 0) 100%) !important;
    padding-bottom: 24px;
